...
if (in_array($rol, [1, 2, 3, 5, 6, 7])) {
  $menu[] = [ 'label' => 'NOTIFICACIONES', 'icon' => 'notifications', 'id' => 'notificaciones_group',
    'children' => [
      ['icon' => 'assignment', 'label' => 'Alistamiento', 'url' => '../despacho/historial_solicitudes_alistamiento.php'],
      ['icon' => 'assignment_ind', 'label' => 'Equipos Asignados', 'url' => '../bodega/asignar.php']
    ]
  ];
}
...
if (in_array($rol, [1, 2, 3, 4, 5, 6, 7])) {
  $menu[] = [
    ...
    'children' => [
      ...
      [ 'label' => '1° TRIAGE', 'icon' => 'assignment_turned_in', 'class' => 'style-triage1',
        'children' => [
      ...
      [ 'label' => '2° TRIAGE', 'icon' => 'assignment_late', 'class' => 'style-triage2',
        'children' => [
          ['label' => '▹ Ingresar Triage 2', 'url' => '../bodega/triage2.php'],
          ['label' => '◇ Histórico Triage 2', 'url' => '../bodega/lista_triage_2.php'],
          ['label' => '▹ Asignar Técnico MYL', 'url' => '../bodega/asignar.php']
        ]
      ],
      // 3.3. Mantenimiento y Limpieza
      [ 'label' => 'MANTENIMIENTO Y LIMPIEZA', 'icon' => 'build', 'class' => 'style-mantenimiento',
        'children' => [
          ['label' => 'Listado de Equipos', 'url' => '../laboratorio/mostrar.php'],
          ['label' => 'Lista de Partes', 'url' => '../bodega/lista_parte.php'],
          ['label' => 'Solicitar Parte', 'url' => '../bodega/solicitar_parte.php'],
          ['label' => 'Historial Mantenimiento', 'url' => '../laboratorio/historial_lab.php']
        ]
      ],
      // 3.4. Reparación Eléctrica
      [ 'label' => 'ELÉCTRICO', 'icon' => 'electrical_services', 'class' => 'style-electrico', 
        'children' => [
      ...
      [ 'label' => 'ESTÉTICO', 'icon' => 'palette', 'class' => 'style-estetico',
        'children' => [
      ...
      [ 'label' => 'CONTROL DE CALIDAD', 'icon' => 'verified', 'id' => 'control_calidad_group', 'class' => 'style-control-calidad',
        'children' => [
...
// ALISTAMIENTOS (Preparación de equipos para venta)
// ========================================================================
$alistamientoItems = [];
if (in_array($rol, [1, 3, 4, 5, 6, 7])) {
    $alistamientoItems[] = [ 'label' => 'Crear Alistamiento', 'icon' => 'add_circle', 'url' => '../venta/preventa.php' ];
    $alistamientoItems[] = [ 'label' => 'MI Solicitud', 'icon' => 'history', 'url' => '../venta/historico_preventa.php'];
}
...
// Menu Padre, sección ALISTAMIENTOS
...
$comercialMenu = [];
if (in_array($rol, [1, 3, 4, 5, 6, 7])) {
  $comercialMenu[] = ['label' => 'Panel Principal', 'icon' => 'dashboard', 'url' => '../comercial/escritorio.php'];
  $comercialMenu[] = ['label' => 'Lista de Clientes', 'icon' => 'list', 'url' => '../clientes/mostrar.php'];
  $comercialMenu[] = ['label' => 'Nuevo Cliente', 'icon' => 'group_add', 'url' => '../clientes/nuevo.php'];
  $comercialMenu[] = [
    'label' => 'Catalogo',
    'icon' => 'auto_stories',
    'url' => 'null',
    'children' => [
      ['label' => '1 ▪ B. ROOM ', 'icon' => 'auto_stories', 'url' => '../b_room/mostrar.php'],
      ['label' => '2 ▪ Catalogo Ventas ', 'icon' => 'auto_stories', 'url', 'url' => '../venta/catalogo.php']
    ]
  ];
  $comercialMenu[] = ['label' => 'Nueva Venta', 'icon' => 'add_shopping_cart', 'url' => '../comercial/nueva_venta.php'];
  $comercialMenu[] = ['label' => 'Crear Alistamiento',  'icon' => 'add_circle', 'url' => '../venta/preventa.php'];
  $comercialMenu[] = ['label' => 'Mi Solitud', 'icon' => 'history', 'url' => '../venta/historico_preventa.php'];
}
// VISTA PANEL ADMIN VISTA COMPLETA TODOS LAS SOLICITUDES DE ALISTAMIENTO
if (in_array($rol, [1, 3, 5, 6, 7])) {
  $comercialMenu[] = ['label' => 'ALISTAMIENTOS', 'icon' => 'playlist_add_check', 'url' => '../despacho/historial_solicitudes_alistamiento.php'];
}
if (in_array($rol, [1, 3, 4, 5, 6, 7])) {
  $comercialMenu[] = ['label' => 'Historial de Ventas', 'icon' => 'receipt_long', 'url' => '../comercial/historico_venta.php'];
  $comercialMenu[] = ['label' => 'Órdenes Pendientes', 'icon' => 'pending_actions', 'url' => '../despacho/pendientes.php'];
  $comercialMenu[] = ['label' => 'Historico Despacho', 'icon' => 'local_shipping', 'url' => '../despacho/historial.php'];
}
if (in_array($rol, [1, 3, 5, 6, 7])) {
  $comercialMenu[] = [ 'label' => 'Marketing', 'icon' => 'campaign', 'url' => '../marketing/mostrar.php'];
}
// Seccion padre Menu Comercial 
...
if (in_array($rol, [1, 2, 3, 4])) {
  $contabilidadItems[] = [ 'label' => 'Ingresos', 'icon' => 'trending_up', 'url' => '../ingresos/mostrar.php' ];
  $contabilidadItems[] = [ 'label' => 'Gastos Generales', 'icon' => 'account_balance_wallet',
    'children' => [
...
if (in_array($rol, [1, 3])) {
  ...
  $reportesItems[] = [ 'label' => 'Estadísticas Técnicos', 'icon' => 'bar_chart', 'url' => '../bodega/graficos_tecnicos.php' ];
}
// 8.3. Gráficos Generales
// Roles: Admin, Contable
if (in_array($rol, [1, 3])) {
  $reportesItems[] = [ 'label' => 'Gráficos', 'icon' => 'show_chart', 'url' => '../graficos/mostrar.php'];
}
// MUENU PADRE | Agregar sección ANÁLISIS Y REPORTES 
...
if (!in_array($rol, [0])) {
  $adminItems[] = [ 'label' => 'Docs Generales', 'icon' => 'library_books', 'url' => '../docs/mostrar.php' ];
}
// 9.2. Gestión de Usuarios
// Roles: Admin, Contable, Jefe Técnico
if (in_array($rol, [1, 3, 5])) {
  $adminItems[] = [ 'label' => 'Usuarios', 'icon' => 'manage_accounts', 'url' => '../usuario/mostrar.php' ];
// 9.3. Configuración del Sistema
  $adminItems[] = [ 'label' => 'Configuración', 'icon' => 'settings', 'url' => '../cuenta/configuracion.php' ];
}
// 9.4. Mi Perfil
