...
if (isset($_POST['action'])) {
    ...
    switch ($_POST['action']) {
        case 'buscar_cliente':
            ...
            $sql = "SELECT idclie, numbid, nomcli, apecli, correo, celu, dircli, idsede
                    FROM clientes
                    WHERE numbid LIKE ? OR nomcli LIKE ? OR apecli LIKE ? OR correo LIKE ? OR celu LIKE ?
                    LIMIT 20";
            ...
            while ($row = $result->fetch_assoc()) {
                $clientes[] = [
                    'id' => $row['idclie'],
                    'text' => $row['nomcli'] . ' ' . $row['apecli'] . ' - ' . $row['numbid'],
                    'numbid' => $row['numbid'],
        ...
        case 'buscar_inventario':
            ...
            $sql = "SELECT id, producto, marca, modelo, procesador, ram, disco, grado, precio_venta
        ...
        case 'guardar_venta':
            try {
                ...
                $canal_venta = mysqli_real_escape_string($conn, $_POST['canal_venta'] ?? 'Tienda Física');
