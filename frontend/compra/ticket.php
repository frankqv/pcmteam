...
while($a = $stmt->fetch()){
    ...
    $pdf->MultiCell(0,5,convertUtf8("Fecha:".date("d/m/Y") ),0,'C',false);
     ...
     $id = $_GET['id'];
    $stmt = $connect->prepare("SELECT compra.idcomp, compra.user_id, compra.method, compra.total_products, compra.total_price, compra.placed_on, compra.payment_status, compra.tipc FROM compra WHERE compra.idcomp= '$id'");
$stmt->setFetchMode(PDO::FETCH_ASSOC);
$stmt->execute();
while($row = $stmt->fetch()){
    ...
    $pdf->MultiCell(0,5,convertUtf8(strtoupper("Ticket Nro:". $row['idcomp'] )),0,'C',false);
    $pdf->SetFont('Arial','',9);
    ...
    # Codigo de barras #
    $pdf->Code128(5,$pdf->GetY(),"COD000001V000".$row['idcomp'],70,20);
    $pdf->SetXY(0,$pdf->GetY()+21);
    $pdf->SetFont('Arial','',14);
    $pdf->MultiCell(0,5,convertUtf8("COD000001V000".$row['idcomp']),0,'C',false);
}
