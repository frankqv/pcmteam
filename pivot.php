...
if ($use_triages) {
    ...
    // Si existe bodega_diagnosticos (este es tu esquema actual según EstructuraDB.sql)
    // Map: fecha_diagnostico -> triage_fecha, estado_reparacion -> triage_estado
    $sql = "
    SELECT 
        inv.id,
        inv.codigo_g,
    ...
    <div class="table-responsive">
        <table id="triageTable" class="table table-striped table-bordered" style="width:100%">
            ...
            <?php while($row = $result->fetch_assoc()): ?>
                <tr data-tecnico="<?= (int)($row['tecnico_triage'] ? $row['tecnico_triage'] : 0) ?>">
                    ...
                    <td>
                        <a href="triage_2.php?id=<?= (int)$row['id'] ?>" class="btn btn-info btn-sm" title="Ver triage">
...
$(document).ready(function() {
    ...
    $('#filterTecnico').on('change', function(){
        ...
        // filtramos por contenido en la columna "Técnico Triag." (índice 8)
        table.column(8).search( '^' + $(this).find('option:selected').text() + '$', true, false ).draw();
    });
});
</script>
</body>
